...
这样，在后续编译rust程序时，将采用上述版本的rust toolchhains。重新执行`build.sh`，发现我们的内核已经可以正确编译了。接下来的任务就是将他和 bootloader 链接，得到可以被 qemu 加载的 os 。
...
> 注意：如果发现没法退回qemu-system-riscv32到shell，则可以在另外一个shell窗口执行命令`killall qemu-system-riscv32` ，就可以打断qemu-system-riscv32的执行，回到shell界面。
