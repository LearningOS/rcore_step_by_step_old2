...
use crate::memory_set::{attr::MemoryAttr, handler::ByFrame, MemorySet};
use crate::process::{ExitCode, Tid};
...
impl Thread {
    ...
    pub unsafe fn new_user(data: &[u8]) -> Box<Thread> {
        ...
        let mut ustack_top = {
            // 创建用户栈
            let (ustack_buttom, ustack_top) =
                (USER_STACK_OFFSET, USER_STACK_OFFSET + USER_STACK_SIZE);
            let paddr = alloc_frames(USER_STACK_SIZE / PAGE_SIZE)
                .unwrap()
                .start_address()
                .as_usize();
            vm.push(
                // 创建一个内核栈之后还需要将这个内核栈装入虚存系统。
                ustack_buttom,
                ustack_top,
        ...
        Box::new(Thread {
            ...
            kstack: kstack,
            proc: Some(Arc::new(Process { vm: Arc::new(vm) })),
        })
    }
...
impl ElfExt for ElfFile<'_> {
    fn make_memory_set(&self) -> MemorySet {
        ...
        for ph in self.program_iter() {
            ...
            // Get target slice
            let target = {
                // 可以看到，这里的 virt_addr 是根据文件中的虚拟地址得到的，所以 target 应该仅用于 with 函数中
            ...
            unsafe {
                ms.with(|| {
                    // with 函数的作用是，将当前这个未激活页表激活并执行一个函数，然后切换回原来的页表
                    if data.len() != 0 {
