...
pub fn load_elf() -> usize {
    let data = unsafe {
        slice::from_raw_parts(
            _payload_start as usize as *const u8,
            _payload_end as usize - _payload_start as usize,
        )
    };
    ...
    for ph in elf.program_iter() {
        ...
        if ph.file_size() > 0 {
            ...
            let target = unsafe { slice::from_raw_parts_mut(ph.virtual_addr() as *mut u8, size) };
...
pub fn load(entry: usize, hartid: usize, dtb: usize) -> ! {
    ...
    unsafe {
        ...
        let mask: usize = 1;
        asm!("mret": : "{x10}"(hartid) "{x11}"(dtb) "{x12}"(mask): : "volatile" );
    }
